...
import { Search, Plus, Eye, Pencil, Trash2, X, ChevronDown } from "lucide-react";
import {
...
const PRODUCT_IMGS: Record<number, string> = {
  ...
  7:"https://images.unsplash.com/photo-1473966968600-fa801b869a1a?w=80&q=70",8:"https://images.unsplash.com/photo-1586790170083-2f9ceadc732d?w=80&q=70",
  9:"https://images.unsplash.com/photo-1638247025967-b4e38f787b76?w=80&q=70",10:"https://images.unsplash.com/photo-1525966222134-fcfa99b8ae77?w=80&q=70",
  11:"https://images.unsplash.com/photo-1614252235316-8c857d38b5f4?w=80&q=70",12:"https://images.unsplash.com/photo-1533867617858-e7b97e060509?w=80&q=70",
  13:"https://images.unsplash.com/photo-1608256246200-53e635b5b65f?w=80&q=70",14:"https://images.unsplash.com/photo-1603487742131-4160ec999306?w=80&q=70",
  15:"https://images.unsplash.com/photo-1515372039744-b8f02a3ae446?w=80&q=70",16:"https://images.unsplash.com/photo-1594938298603-c8148c4b4357?w=80&q=70",
  17:"https://images.unsplash.com/photo-1487222477894-8943e31ef7b2?w=80&q=70",18:"https://images.unsplash.com/photo-1434389677669-e08b4cac3105?w=80&q=70",
  19:"https://images.unsplash.com/photo-1509631179647-0177331693ae?w=80&q=70",20:"https://images.unsplash.com/photo-1539109136881-3be0616acf4b?w=80&q=70",
  21:"https://images.unsplash.com/photo-1485968579580-b6d095142e6e?w=80&q=70",22:"https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=80&q=70",
  23:"https://images.unsplash.com/photo-1515347619252-60a4bf4fff4f?w=80&q=70",24:"https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=80&q=70",
};

const GENDERS: (Gender | "all")[] = ["all", "men", "women", "teens", "kids"];
const TYPES: (ProductType | "all")[] = ["all", "clothing", "footwear"];
...
type FormState = {
  ...
  colors: string[]; sizes: string[]; tag: string; stock: string;
...
export default function AdminProducts() {
  ...
  function openEdit(p: typeof ALL_PRODUCTS[0]) {
    setSelected(p);
    setForm({ name: p.name, description: "", price: String(p.price), discountPrice: "",
      gender: p.gender, type: p.type, subtype: p.subtype,
      colors: p.colors, sizes: p.sizes, tag: p.tag ?? "", stock: "24", sku: `VXE-${p.id.toString().padStart(4,"0")}`, status: "Active" });
    setModal("edit");
  }
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-amber-tan/8">
                {["Product", "Gender", "Type / Subtype", "Price", "Stock", "Tag", "Status", ""].map((h) => (
                ...
                <tr key={p.id} className={`hover:bg-amber-tan/3 transition-colors ${i < filtered.length - 1 ? "border-b border-amber-tan/6" : ""}`}>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-dm text-[10px] text-linen-cream/30 mt-0.5">SKU: VXE-{p.id.toString().padStart(4,"0")}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3.5 font-dm text-xs text-linen-cream/50 capitalize">{p.gender}</td>
                  <td className="px-5 py-3.5 font-dm text-xs text-linen-cream/50 capitalize">{p.type} · {p.subtype}</td>
                  <td className="px-5 py-3.5 font-dm text-sm text-amber-tan font-medium">{formatNGN(p.price)}</td>
                  <td className="px-5 py-3.5">
                    <span className="font-dm text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full">24</span>
                  </td>
                  <td className="px-5 py-3.5">
                    {p.tag ? (
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2">
                      <button onClick={() => openView(p)} className="text-linen-cream/20 hover:text-amber-tan transition-colors"><Eye size={14} /></button>
                      <button onClick={() => openEdit(p)} className="text-linen-cream/20 hover:text-amber-tan transition-colors"><Pencil size={14} /></button>
                      <button onClick={() => setDeleteId(p.id)} className="text-linen-cream/20 hover:text-red-400 transition-colors"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/80 backdrop-blur-sm overflow-y-auto py-8 px-4">
          <div className="card-gloss rounded-2xl w-full max-w-2xl border border-amber-tan/15">
            ...
            <div className="p-6 space-y-5">
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea value={form.description} onChange={(e) => setF("description", e.target.value)}
                    rows={3} placeholder="Product description..." className="input-gold w-full px-4 py-2.5 rounded-sm font-dm text-sm resize-none" />
                </div>
                <div>
                  <Label>Price (₦)</Label>
                ...
                <div>
                  ...
                  <input type="number" value={form.discountPrice} onChange={(e) => setF("discountPrice", e.target.value)}
                    placeholder="75000" className="input-gold w-full px-4 py-2.5 rounded-sm font-dm text-sm" />
                </div>
                <div>
                  <Label>Gender</Label>
                  <Select value={form.gender} onChange={(v) => { setF("gender", v); setF("subtype", ""); setF("sizes", []); }}>
                    {(["men","women","teens","kids"] as Gender[]).map((g) => <option key={g} value={g} className="capitalize">{g}</option>)}
                  </Select>
                </div>
                <div>
                  <Label>Type</Label>
                  <Select value={form.type} onChange={(v) => { setF("type", v); setF("subtype", ""); setF("sizes", []); }}>
                    {(["clothing","footwear"] as ProductType[]).map((t) => <option key={t} value={t} className="capitalize">{t}</option>)}
                  </Select>
                </div>
                <div>
                  <Label>Subtype</Label>
                  <Select value={form.subtype} onChange={(v) => setF("subtype", v)}>
                ...
                <div>
                  <Label>Tag</Label>
                  <Select value={form.tag} onChange={(v) => setF("tag", v)}>
                ...
                <div className="flex flex-wrap gap-2">
                  {COLORS.map((c) => (
                    <button key={c.name} type="button" onClick={() => setF("colors", toggleArr(form.colors, c.name))}
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border font-dm text-xs transition-colors ${form.colors.includes(c.name) ? "border-amber-tan text-amber-tan bg-amber-tan/10" : "border-amber-tan/15 text-linen-cream/40 hover:border-amber-tan/40"}`}>
                ...
                <div className="flex flex-wrap gap-2">
                  {availableSizes.map((s) => (
                    <button key={s} type="button" onClick={() => setF("sizes", toggleArr(form.sizes, s))}
                      className={`px-3 py-1.5 border font-dm text-xs rounded-sm transition-colors ${form.sizes.includes(s) ? "bg-amber-tan text-obsidian border-amber-tan font-semibold" : "border-amber-tan/15 text-linen-cream/40 hover:border-amber-tan/40"}`}>
                      {s}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <Label>Images</Label>
                <div className="border-2 border-dashed border-amber-tan/15 rounded-lg p-8 text-center hover:border-amber-tan/30 transition-colors cursor-pointer">
                  <p className="font-dm text-sm text-linen-cream/30">Click to upload or drag & drop</p>
                  <p className="font-dm text-xs text-linen-cream/20 mt-1">PNG, JPG up to 5MB each</p>
                </div>
              </div>
            </div>
            <div className="flex gap-3 px-6 py-5 border-t border-amber-tan/10">
              ...
              <button onClick={handleSave} className="flex-1 btn-amber sheen py-3 text-obsidian font-dm font-semibold text-[11px] tracking-[0.15em] uppercase rounded-sm">
          ...
          <div className="card-gloss rounded-2xl w-full max-w-md border border-amber-tan/15 p-6">
            ...
            <div className="space-y-3 font-dm text-sm">
              {[
                ...
                <div key={k} className="flex justify-between border-b border-amber-tan/6 pb-2">
                  <span className="text-linen-cream/35">{k}</span>
